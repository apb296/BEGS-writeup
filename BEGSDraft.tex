...
Our first set of results extends a Ricardian equivalence type of reasoning to argue that gross asset positions (in particular the level of government debt) do not affect the set of feasible allocations that can be implemented in competitive equilibria with taxes and transfers. For example, an increase in government debt that is shared equally by all agents and thus leaves net assets unchanged  has no welfare consequences, in line with the principles proclaimed by Simon \citet{newcomb1865critical} %Simon Newcomb (1865)
...
%  In section \ref{Sec: quasilinear}, we use these Bellman equations to characterize an optimal equilibrium in an economy with quasilinear preferences. In section \ref{sec: longrun anatyt},  we use them to study asymptotic outcomes under more general preferences.     In section \ref{Sec: numerical}, we use numerical solutions of these Bellman equations to study a calibrated economy.
 ...
 We return to the more general  problem  formulated in section \ref{Sec: environment}. We further assume that $U^{i}:\mathbb{R}%
...
l}_{i}}U_{l}^{i}\left( c,x\right) =\infty $ and $\lim_{x\rightarrow
...
Constraint \eqref{eq imp sum t=1} imposes a measurablity restriction on  allocations asserting  that the right side is determined at time $t-1$.
...
For $I=2$, proposition \ref{prop: long run forces}  signs the marginal utility adjusted net assets which we denoted by $x$. This implies a particular ordering of net assets across the agents in the steady state. These implications generalize to settings with $I\geq3$ too. In general, one can verify that without discount factor shocks (as in  part 1 of proposition \ref{prop: long run forces} when interest rates are countercyclical), the steady state net asset  levels (scaled by marginal utilities) are ordered inversely relative to productivities. By having the net asset positions negatively correlated with wage earnings, the planner can minimize the costs of fluctuating transfers. Further for similiar reasons as in the $I=2$ case,  with large enough spread in discount factors that makes interest rates procyclical, and the net asset positions are positively correlated with productivities.
...
rises, and the discount factor is  high. We set  initial  government debt to be 60\% of GDP, roughly to match the ratio of federal debt held by public at the
beginning of 2010.
...
the government holds claims on the private sector when the state is  in the ergodic set. Consistent with Proposition \ref{prop: long run forces} %XXXXXX, \textcolor{red}{Please nail down the
 %section number.}
...
The four panels show tax rates, transfers, debt and interest rate movements for
a path of 25 years.
...
higher welfare in the economy with ad-hoc borrowing limits. We restrict ourselves to a deterministic economy with $g_t=0$, $\beta_t=\beta$ and $I=2$.  The the planners problem can then be written as the following sequence problem
\begin{equation}
...
\end{align}
\end{subequations}  Where $\underline b_i$ is the exogenous borrowing constraint for agent i.  Let $c^{fb}_i$ and $l^{fb}_i$ be the allocation that solves the first best problem, that is maximizing equation \eqref{eq.A_obj} subject to \eqref{eq.A_feas}, and define
\begin{equation}
...
\end{equation}  We will assume that the exogenous borrowing constraints constraints satisfy $\underline b_2 = \underline b_1 + \tilde b_2^{fb}$.  We then have the following lemma
...
This will improve upon the planners problem with natural borrowing constraints, as first best can only be achieved in this scenario when $b_{2,-1}-b_{1,-1} = \tilde b_2^{fb}$.
...
[\mu\frac1{c_2}]}{\mathbb{E}[\frac1{c_2}]} \\ \label{eq.x_env}
...
\rho^{\prime }(s) = \rho$ and $x^{\prime }(s) = x$. It should be clear that if we replace $\mu(s) = \mu$, equation (\ref{eq.x'FOC})  and equation (\ref{eq.x_env})is
