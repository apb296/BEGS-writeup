...
%drift to zero. Optimal labor distortions and debts in a heterogenous agent
...
%government debt. If that restriction were imposed in our heterogenous agent
...
While results in sections \ref{Sec: characterization}, \ref{Sec: quasilinear}, \ref{sec: longrun anatyt}, and  \ref{Sec: numerical}  depend on these  assumptions about an affine tax system and incomplete markets,
...
See \citet{Yared2010,Yared2013}  shows a closely related result.}
\smallskip
...
\subsection{The optimal response to government expenditure shocks}\label{sec: quasi linear interior }
...
weight on low productivity types and the shocks to $g$ are not too high.\footnote{ In
  section \ref{sec: Non Interior Eqb}, we show these conditions explicitly when agents have CES disutility from labor.} To simplify notation, we now assume that the initial debt is $%
...
%heterogenous agent economy (dotted line) with quasi-linear preferences}}{%
...
\smallskip
%\textcolor{red}{Anmol: good job on your editing. I did some more editing.}
...
lower bound on consumption and faces a high marginal cost. %\textcolor{red}{Anmol:  the previous sentence is unclear.  What is the `or' to complete
%the `either'?}
...
%\textcolor{red}{XXXXX I edited the following paragraph.}
...
%\textcolor{red}{XXXXXX Anmol and David -- I did some more editing of Anmol's good editing of June 18, 2013.}
...
%\textcolor{red}{Tom XXXXX: write a transition here.}
%\textcolor{red}{Tom XXXXX: stopped here, June 26.}
%\textcolor{blue}{Anmol : I moved a few sentences to make a transition}
...
\end{proposition}
%
%\textcolor{red}{Anmol XXXXXX: please look at how I edited the proposition.  Do you want to add the pointers to the
%figures as we discussed in the expensive coffee shop at SNU?}
%
%\textcolor{blue}{Anmol : I think the pointers to figure that comes much later is a bit confusing. Its better to explain the connection to this proposition when we discuss the three lines later in the numerical section}

\smallskip  The key step of the proof, which appears in the appendix,  is
to show that there exists $\left( x,\rho \right) $ such that condition (\ref%
{eq:BM2_Imp_cons}) is satisfied for all $s.$
...
times of increased need for government revenues, as in
 ...
 %\textcolor{red}{From Tom: I did a lot of surgery in the above paragraph and earlier in the section too.}

 %In that case, the government finance some of the required  extra revenues by its earnings from
%its assets.
%\textcolor{red}{From Tom: I have edited the above paragraph.  There is a new concept introduced here -- `needed government revenues'. This is
%seemingly distinct from g and includes transfers (and interest payments?).  I am a little worried that it is vague and the term is used in different ways in different places. Let's talk about this.}
%\textcolor{blue}{From anmol : I think we agreed on this terminology to have a parsimonius label for ``states'' when expenditure is high or productivity is low. Both these states are periods when the government requires additional revenues to either finance higher outflows  or make up for the lower tax base }

Depending on  details of shock processes, these two forces can
either reinforce each other (as happens in the Part 1 %\textcolor{red}{From Tom: can we label the part more precisely? Part 1?}
...
generally.  The only
place in the proof where we used the particular form of utility functions  was to prove an existence of a
solution to a particular equation that with our log-quadratic specification is  a quadratic that is
easy to solve analytically.  For other types of preferences, existence of a fixed point of the pertinent equation
can be  verified numerically.\footnote{Appendix \ref{apndx: numerical methods} describes a numerical test for existence and local stability cast in terms of  primitives.} When there are more than two possible values for the shocks or when
...
described in Proposition \ref{prop: long run forces}. We will discuss these
issues in more details in the  section \ref{Sec: numerical}.
...
Section \ref{sec: longrun anatyt} showed that the long run policy
...
the government has accumulated assets in this ergodic set. As alluded to in section \ref{sec: longrun anatyt} %XXXXXX, \textcolor{red}{Please nail down the
 %section number.}
...
\mathbb{E}[\frac{1}{c_{2}}](\rho_{SS})}$ such that
\begin{equation}
1>\frac{1+\rho_{SS}[l_{1}(\rho_{SS},s_{l})^{2}-1]}{1+\rho_{SS}[l_{1}(\rho_{SS},s_{h})^{2}-1]}=\frac{\frac{1/c_{2}(\rho_{SS},s_{l})%
}{\mathbb{E}[\frac{1}{c_{2}}](\rho_{SS})}-\beta (s_{l})}{\frac{%
1/c_{2}(\rho_{SS},s_{h})}{\mathbb{E}[\frac{1}{c_{2}}](\rho_{SS})}%
...
1/c_{2}(\rho_{SS},s_{l})}{\mathbb{E}[\frac{1}{c_{2}}](\rho_{SS})}%
-\beta (s_{l})}<0
\end{equation*}%
...
1/c_{2}(\rho ,s_{l})}{\mathbb{E}[\frac{1}{c_{2}}](\rho )}>\frac{1/c_{2}(\rho
,s_{h})}{\mathbb{E}[\frac{1}{c_{2}}](\rho )}$, in order for equation (\ref{eq.beta_cond}) to hold we need $\beta _{l}>\beta _{h}$. We can then rewrite
equation (\ref{eq.beta_cond}) as
\begin{equation*}
1>\frac{\beta (s_{h})}{\beta (s_{l})}>\frac{\frac{1/c_{2}(\rho_{SS},s_{l})}{\beta
_{l}\mathbb{E}[\frac{1}{c_{2}}](\rho_{SS} )}-1}{\frac{1/c_{2}(\rho_{SS} ,s_{h})}{\beta
(s_{h})\mathbb{E}[\frac{1}{c_{2}}](\rho_{SS} )}-1}
\end{equation*}%
Thus
\begin{equation}
R(s_{l})\frac{1/c_{2}(\rho_{SS} ,s_{l})}{\beta (s_{l})\mathbb{E}[\frac{1}{c_{2}}%
](\rho_{SS} )}<\frac{1/c_{2}(\rho_{SS} ,s_{h})}{\beta (s_{h})\mathbb{E}[\frac{1}{c_{2}}%
](\rho_{SS} )}=R(s_{h})
\end{equation}%
