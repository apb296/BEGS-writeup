...
\begin{itemize}
\item The automatic figure numbering isn't working.  Someone please look into it.
...
\begin{equation}\label{eqn:optPP}
 ...
 Equation \eqref{eqn:optPP} lets us depict an optimal payoff matrix as a function of initial government debt.  Figure \ref{fig:optP} plots the optimal payoff in both states of the world when either government expenditures or TFP follows a 2 state i.i.d. process. In both cases, we see that the ordering of the payoff flips on either side of zero government debt.
\begin{figure}[ht]\label{fig:optP}
		\begin{center}
		\includegraphics[scale=.35]{Images/p_graph_tfp.eps}
		\includegraphics[scale=.35]{Images/p_graph.eps}
		\caption{Optimal asset payoff structure as a function of initial government debt when TFP follows a 2 shock i.i.d. process (left) and when government expenditures follow a 2 shock i.i.d process (right).}
	\end{center}	
\end{figure}
...
\begin{remark}
		...
		\end{remark}
		We illustrate Propostion \ref{prop:ssexistence} in figure \ref{fig:noStable}.  The  blue curve is the inverse map $\bm p^{*-1}$.  Two constants $\alpha_1$ and $\alpha_2$ divide  possible payoff structures into three regions: one in which a steady state exists with the government holding assets,
...
\caption{Three regions in $\bm{p}$ space.}
	\end{center}	
	\end{figure}
...
\end{remark}}
Figure \ref{fig:conv_RA} plots a simulation of the Ramsey plan.  The path of marginal utility weighted government debt resembles the path of government debt for tje quasilinear economy with low $\bm p$ plotted earlier in Figure \ref{fig:2ps_figure}.
\begin{figure}[ht]\label{fig:conv_RA}
	\begin{center}
	\includegraphics[width=4in]{Images/2stateiid.eps}
\caption{Sample path of $x_t$  for an economy with risk aversion and a 2 state i.i.d. process for \tjs{David: please say more about this economy -- is $g$ random or TFP?}}
	\end{center}
\end{figure}
