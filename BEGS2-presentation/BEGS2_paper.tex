...
The government and the consumer trade a single  possibly risky  asset whose payoffs are described the
the  $S \times S$ matrix $\mathbb{P}$ with the time $t$ payoff being
\[p_t=\mathbb{P}(s_{t}|s_{t-1}).\] 
We let $B_t$ denote the government's holdings of the asset and $b_t$ be the consumer's holdings.  
