...
\subsection{Interest in quasi-linear $U$}
...
We execute this three step process as follows.
(1)
...
In this case, $\mathbb{P}(s\_,s)$ is independent of $s\_$, so that $\mathbb{P}$ can be taken to be  a vector.
 ...
 and a risk-free bond is a security for which $\bm{p}=1$.
We take $\bm{p}$ to be the asset's payoff in the ``good'' state $s$. 
...
Suppose that $s$ shocks affect either $g$ or $\theta$.  Then there exist $0 \geq \alpha_2\geq\alpha_1\geq1$
such that
  \begin{itemize}
...
\end{proposition}
\begin{proof}
\textcolor{red}{David XXXXX: please fill in and refine.}
...
After the realization of time time $0$ Markov shock $s_0$, let $W(b_{-1},s_0)$ be the value of the Ramsey plan when initial
government debt is $b_{-1}$. % These value functions satisfy  two
%Bellman equations. 
An (\textit{ex ante})} Bellman equation for $t\geq1$  is
	\begin{equation}
...
subject to $x'(s)\in [\underline x,\overline x]$ and
	...
	\end{align}
	Given an initial  debt $b_{-1}$,time $0$ Markov state $s_0$,  and continuation value function $V(x,s\_)$, an (\textit{ex post}) time $0$ Bellman equation
