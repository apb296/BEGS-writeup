...
What is an optimal  government debt? And  is government debt  a pertinent state variable? % in a recursive formulation of a Ramsey problem?
...
\dge{With equation \eqref{eqn:optPP} we can easily plot the optimal payoff matrix as a function of initial government debt.  In figure \ref{fig:optP} we plot the optimal payoff in both states of the world when either government expenditure or TFP follows a 2 state i.i.d process.}
\begin{figure}[ht]\label{fig:optP}
		\begin{center}
		\includegraphics[scale=.35]{Images/p_graph_tfp.eps}
		\includegraphics[scale=.35]{Images/p_graph.eps}
		\caption{Optimal asset payoff structure as a function of initial government debt when TFP follows a 2 shock i.i.d process (left) and government expenditure follows a 2 shock i.i.d process (right)}
	\end{center}	
\end{figure}\dge{In both cases we see that the ordering of the payoff flips on either side of zero government debt.} 
...
\begin{remark}
		With only TFP shocks, the bond payoff has the special property that it is associated with a steady state asset level that supports the first-best allocation, $\bm p^{* -1}(1) = b_{fb}$.  At \st{a}\dge{the} first-best taxes are zero, so the net-of-interest government surplus is constant across states.
		\end{remark}
		\dge{We illustrate the results of propostion \ref{prop:ssexistence} in figure \ref{fig:noStable} below.  In blue we have ploted the inverse map $\bm p^{*-1}$.  As in the proof there are two constants $\alpha_1$ and $\alpha_2$ which split the possible payoff structures into three regions: where a steady state exists with government holding assets, where a steady state exists with government holding debt, and where a steady state does not exist.}
		\begin{figure}[ht]\label{fig:noStable}
		\begin{center}
...
\begin{description}
	\item[1. $\bm p < \min\{1,\alpha_1\}$:]  Let $\overline b_s$ be maximal debt with which the government %\st{good}
...
\end{description}
\end{proof}
\dge{\begin{remark}  The results of Theorem \ref{thm:convergence} are illustrated in Figure \ref{fig:stable}.  In addition to denoting the values of $\bm p$ for which a steady state exists we also highlight the regions where this steady state is stable.  One feature of this theorem is that there exists $\bm p$ where a steady state exists but economy will not converge to the steady state allocation in the long run.
\end{remark}}
\begin{figure}[ht]\label{fig:stable}
		\begin{center}
		\includegraphics[width = 5in]{Images/graph_stable.pdf}
\caption{Stability regions in $\bm p$ space.}
	\end{center}	
...
\begin{figure}\label{fig:2ps_figure}
	\begin{center}
	\includegraphics[width=3in]{Images/port1.eps}
	\includegraphics[width=3in]{Images/port2.eps}
\caption{A sample path with $\bm p > 1$ (left)  $\bm{p} <1$ (right)}
	\end{center}	
\end{figure}
...
and $s_g$ begin the good state.
		\dge{Let $x_{fb}$ be the discounted present value of marginal utility weighted government surpluses associated with the first best allocation.} \st{ be a value of  %\st{the state $x$ from which a government can implement first=best with complete markets}
...
\end{remark}}
\dge{In Figure \ref{fig:conv_RA} we plot a simulation of the optimal policy.  We note that the path of marginal utility weighted government debt is very similar to the path of government debt for a quasilinear economy with low $\bm p$ plotted in Figure \ref{fig:2ps_figure} }
\begin{figure}[ht]\label{fig:conv_RA}
	\begin{center}
