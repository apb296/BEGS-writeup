...
\begin{itemize}
\item [*] Answers under polar assumptions:  LS -- complete markets; AMSS --- a risk-free bond only
...
\begin{itemize}
	\item  A unit of government debt  pays off $p_s$ in state $s$ next period
\end{itemize}
...
at $t = ????$ \textcolor{red}{David and Anmol XXXXXX: please answer ????}
\begin{enumerate}
...
\begin{frame}
 ...
 \textbf{Risk aversion:}
  \begin{itemize}
   ...
   \item To focus on the \textbf{endogenous component} of payoffs, set  $p(s)=1 \forall s$ \textcolor{red}{XXXX Tom added $\forall s$}
  \end{itemize}
