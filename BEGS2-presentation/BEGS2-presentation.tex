...
the $b$ argument.  See earlier slide where $W$ is defined.}
	\[
		W(b_{-1},s_0) = \max_{c,l,x'} U(c,l) +\beta V(x',s0)
...
\begin{itemize}
	...
	\item If the asset payoff structure offers greater returns in good states of the world then bad, then the Ramsey government  asymptotically accumulates debt \textcolor{red}{Anmol and David XXXXXX: does `accumulate debt' mean `run up debt' or `accumulate assets'?}
