...
\item Why might different economic fundamentals lead governments to want different amounts of debt?
\item Existing answers hinge on polar assumptions:
\begin{itemize}
 \item [+]  Lucas Stokey (1984), complete markets: non history dependent debt quantities inherited from initial debt
 \item [+ ]  AMSS (2002), a risk-free bond only, quasi-linear preferences: govt. accumulates {\em assets} sufficient to finance activities using interest revenues
\end{itemize}
\item Unknown after AMSS (2002): what if interest rates fluctuate?
\end{enumerate}
\end{frame}
...
\end{equation*}%
  \item \textbf{Technology}: Output  $y_t=\theta_{t} l_{t}$
   \end{itemize}
...
\end{itemize}
  ...
  \item \textbf{Linear Taxes}: Representative consumer's  tax bill
...
\item[]
  \item
  \begin{itemize}
   \item \textbf{Feasibility}: $c_{t}+g_t = \theta _{t} l_{t}$

   \item \textbf{Market Clearing for Asset}: $b_{t}+B_{t}=0$
\end{itemize}
  \item[]
...
the  allocation is individually rational, markets clear \footnote{Usually, we impose only  ``natural'' debt limits. }
\end{definition}
...
\begin{itemize}
 \item Given \textbf{arbitrary} initial govt. debt $b_{-1}$, what is  an \textbf{optimal} asset payoff matrix $\mathbb{P}^* =\mathbb{P}^*(b_{-1})$?
...
Given
 initial govt. debt $b_{-1}$,  let $\mu(b_{-1})$ be the Lagrange multiplier on the Lucas-Stokey implementability constraint
\begin{enumerate}
...
\begin{frame}
	\frametitle{Intuition for Convergence Alternate}
	\begin{itemize}
		\item  The actions of the government can be understood by breaking the first order condition of the planner into two components
		\[
			\mu_t = \EE_t\mu_{t+1} + \EE_t\left[(p_{t+1}-1)\mu_{t+1}\right]
		\]
		\item  On the left-hand side we have the marginal benefit of issuing and additional unit of debt.
		\item  The first term on the right-hand side is the cost of issuing that debt if the payoff structure were a bond, the second term is the additional cost/benefit from the correlation of the payoffs with tightness of the government budget constraint.
		\item  If the additional debt pays off more (less) when the government budget constraint is tight (low $\mu$) the planner will issue less (more) debt relative to if had access to a risk free bond.
	\end{itemize}
\end{frame}

\begin{frame}
	\frametitle{Intuition for Convergence Alternate Continued}
	\begin{itemize}
		\item  As the policy for a risk-free bond is a martingale in $\mu_t$ this introduces a drift.
		\item  There are two sources of variation in the governments budget constraint: variation in exogenous factors such as TFP or $g$, and variation in payments on debt.
		\item  At the steady state these are in perfect balance, as government debt level is moved away from the steady state the change in payments on debt term dominates.
		\item  Causes drift toward steady state, except when variation in $\mathbb P$ is small relative to the TFP shocks.
	\end{itemize}
\end{frame}


\begin{frame}
	\frametitle{More than $2$ states}
	\begin{itemize}
		\item  In an i.i.d. economy with $S$ states of the world, the range of the mapping $b_{-1}\mapsto \mathbb P$ is a one dimensional subspace of the $S-1$ dimensional space of possible payoffs.
		\item  While is not possible to solve analytically for the optimal policy rules, we can construct linear approximations to the policy rules.
		\item  We can the characterize the ergodic distribution and convergence rates of the linearized system.
		\item  For simplicity we focus on an i.i.d. process for government expenditure shocks $g_s$, we normalize $\theta_s = 1$.
	\end{itemize}
\end{frame}
...
\begin{frame}
	\frametitle{Method}
	\begin{itemize}
		\item  The standard approach constructing linear approximations to policy rules  is take the limit as the size of the shocks approaches zero.
		\item  This approach misses key features  features of the policy rules, specifically the regions where $\mu_t$ is a sub- and super- martingale.
		\item  Our alternative is to treat the policy rules $c(b;s,\mathbb P)$, $l(b,s;\mathbb P)$, and $b'(b,s;\mathbb P)$ as functions of both the state variable $b$ and the payoff structure $\mathbb P$.
		\item  We can then linearize a steady state $\overline b, \overline {\mathbb P}$ associated with complete markets Ramsey allocation.
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{lemma}\label{lem.cm_payoff}
	 The payoff vector $\overline{\mathbb P}$, normalized so that $\EE\overline{\mathbb P} = 1$, associated with complete markets Ramsey allocation with government debt $\overline b \leq \overline b^n$ ($\overline b^n$ is the natural debt limit) must satisfy
	\begin{equation}\label{eq.pbar}
		\overline {\mathbb P}_s = 1- \frac{\beta}{\overline b}(g_s - \EE g).
	\end{equation}Moreover, the steady state $\overline b$ is globally stable: $b_t\rightarrow \overline b$ with probability 1 for the incomplete markets Ramsey allocation.
\end{lemma}
\pause
\begin{description}
	\item[Intuition: ]  All fluctuations in government surplus must come from $g_s -\EE g$.  The payoff structure $\mathbb P$ is chosen so that the income from debt
	\[
		\overline b\frac{\mathbb P_s -1}{\beta}
	\]perfectly accounts for those fluctuations.
	\end{description}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		\item  For a given $\mathbb P$ choose $(\overline b,\overline{\mathbb P})$ so as to minimize
		\[
			\|\mathbb P-\overline{\mathbb P}\|^2 = \sum_s \Pi_s (\mathbb P_s-\overline{\mathbb P}_s)^2
		\]
		\item  Note $\hat{\mathbb P} = \mathbb P-\overline{\mathbb P}$ is orthogonal to $g$. 
	\end{itemize}
	\pause
\begin{proposition}\label{prop.erg_lin}
Suppose $\mathbb P$ admits a decomposition $\mathbb P = \hat{\mathbb P} +\overline{\mathbb P}$ with $\hat{\mathbb P}$ orthogonal to $g$ and
\[
	\overline{\mathbb P} = 1- \frac{\beta}{\overline b}( g - \EE g).
\] with $\overline b \leq \overline b^n$.  Then the ergodic distribution of debt of the policy rules linearized around $(\overline b, \overline{\mathbb P})$ will have mean $\overline b$ and variance
\begin{equation}\label{eq.var_lin}
	\frac{\overline b^2 \var(\hat{\mathbb P})}{(1+\var(\overline{\mathbb P}))\var(\overline{\mathbb P})}.
\end{equation}
\end{proposition}
\end{frame}
...
\begin{frame}
	\frametitle{Notes}
	\begin{itemize}
	\item  The ergodic distribution of government debt for the linearized system will have mean
	\[
		-\beta\frac{\var(g)}{\cov(\mathbb P,g)}
	\] and coefficient of variation
	\[
		\frac{\sigma_b}{\overline b} = \sqrt\frac{\var(\mathbb P) - |\cov(g,\mathbb P)|}{(1+|\cov(g,\mathbb P)|)|\cov(g,\mathbb P)|}\leq\sqrt\frac{\var(\mathbb P) - |\cov(g,\mathbb P)|}{|\cov(g,\mathbb P)|}
	\]
	\item The speed of convergence to the ergodic distribution given by
	\[
		\frac{\EE_{t-1}(b_t-\overline b)}{(b_{t-1} - \overline b)} = \frac1{1+|\cov(\mathbb P,g)|}
	\]
	\end{itemize}
\end{frame}

\begin{frame}
	\frametitle{Comparisons of Ergodic Distribution}
	\begin{center}
		\includegraphics[width =4in]{Images/erg1.eps}
	\end{center}
\end{frame}
\begin{frame}
 ...
 \begin{enumerate}
  ...
  \item  With more than 2-states the linearized system has an ergodic distribution with mean and variance depend on the variance decomposition of $\mathbb P$ with respect to  $g$.
...
\end{frame}
 ...
 \begin{frame}
	...
	\begin{enumerate}
	 \item Commitment implies that government actions at $t \geq 1$ are constrained by the household's anticipations about them at $s < t$
...
We plan to study more general shocks processes
