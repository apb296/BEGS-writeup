...
\frametitle{Roadmap, the answers}
	...
	\begin{itemize}
	\item We first reverse engineer an optimal $\mathbb{P}^*(b)$ using a Lucas Stokey allocation
	
	 \item In a binary IID world, we identify  $\mathbb{P}$'s for which $b_t$  under a Ramsey policy converges to $b^*$ which solves
	 
	\[\mathbb{P}=\mathbb{P}^*(b^*)\]
	
	
	\item For more general shock structures, we numerically verify  an ergodic set of $b_t$ hovering around $b^*$ which has a property that 
