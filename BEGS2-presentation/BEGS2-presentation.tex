...
\begin{itemize}
 ...
 \item With positive initial assets: Want assets  that pay more in ``adverse'' states
 \item With negative initial assets: Want assets that pay less in ``adverse'' states
 \item
\end{itemize}

 \item \textbf{Nature of shocks: } When do we approximate a ``risk-free'' bond?
 \begin{itemize}
  \item With i.i.d shocks to government expenditures: As assets approach  infinity
  \item With i.i.d  shocks to  TFP: As  allocation  tends to first best
 \end{itemize}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item \textbf{Characterization: } Given an asset payoff structure, with  a 2 state i.i.d process for the aggregate state
		\begin{itemize}
			\item When does a steady state exist? is it unique?
			\item What is the level of government debt of  assets in a steady state?
...
\end{frame}
 ...
 \begin{frame}
  ...
  \begin{itemize}
   ...
   \item Index incomplete market economies by $p_1=p(s=1)$. \footnote{ A risk free bond corresponds to $p_1=1$}
   \item \textbf{Anmol and David XXXXXX: Here should say what $p_2$ is too.}
  \end{itemize}
...
\begin{frame}
 \frametitle{Thresholds: $\alpha_1 <\alpha_2$}
	...
	\begin{itemize}
		\item With nly government expenditure shocks
		\[
 ...
 \begin{frame}
  ...
  \begin{itemize}
		...
		\] or
		\[
		\EE_t  \mu_{t+1}	= \mu_t -\mathbb{Cov}_t p_{t+1} \mu_{t+1}
		\]
...
\begin{frame}
 ...
 \begin{enumerate}
  ...
  \item The level of and sign $b^*$ is a function of the \textbf{exogenous payoff structure}: $p(s)$
 \end{enumerate}
...
\textbf{Anmol and David XXXXX: I exterminated some $\rho$s in this slide.}
Let $\Psi \left( s;{x},s\_\right) $ be an optimal  law of motion for the state variable
for the $t\geq1$ recursive problem, i.e.,


\[\Psi \left( s;{x},%
s\_\right) =\left( x^{\prime }\left( s\right) ,\left( s\right) \right) \]

attains $t\geq1$ value function given state $\left({x},s\_\right) $

\begin{definition}
 A steady state  $\left( {x}^{*}\right) $  satisfies $\left({ x}^{*}\right) =\Psi \left( s;{x}^{*},s_{-}\right) $ for all $%
\,s,s\_$
\end{definition}
\vspace{3mm}
...
\textbf{Anmol and David XXXXXX: something is off or missing in the following statement, especially following `with $\ldots$'.}

	\begin{proposition}  Let $\{c_t(s^t), l_t(s^t), x_t(s^{t-1})\}$ solve the incomplete markets Ramsey problem with.  Then  $x_t(s^{t-1})\rightarrow x^*$ as $t\rightarrow \infty$ with probability 1 for all initial conditions
...
\item  Even with lump sum transfers,  in cases where the steady state exists and is stable, if the initial debt of the government is above its steady state,  the economy  converges with probability 1 to the steady state.
...
\begin{itemize}
	...
	\item  \textbf{Anmol and David XXXXX: I don't understand this one!}
With risk aversion, the payoff structure of the time-consistent savings variable is what determines the long run debt position of the government.
